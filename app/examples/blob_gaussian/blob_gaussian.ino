// SOZO LED visualizer — gaussian circle pattern.
//
// A radial 2D Gaussian centered on the board breathes in and out.
// Brightness falls off smoothly with distance from the center; the
// gaussian's sigma grows and shrinks so the lit region pulses in
// physical size, not just brightness. Slow and ambient.
//
// Uses world (x,y) coordinates so the falloff is spatially symmetric
// regardless of strip-index ordering.
...
const float CX        = 2.0f;     // center x of the gaussian (inches)
const float CY        = 5.0f;     // center y
const float SIGMA_MIN = 3.0f;
const float SIGMA_MAX = 14.0f;
const float BREATH_HZ = 0.22f;
